...
```
Several values systems are embedded in databases I have encountered in my daily life, including the nuclear family, patrilineal surnames and western naming conventions, and generally a binary gender system as the article also discussed. One database I interact often would be related to my student identification at the University of Toronto. The conventional naming system of Last Name, First Name and sometimes a middle name, exists here as in many other databases that contain name information. This naming convention assumes a specific cultural identity in terms of social structure and sense of self. Last Name is related to the family and often passed down by the father. A woman is expected to change their name upon marriage. In other cultures, for example, Spanish naming customs incorporate father's paternal family name followed by the mother's paternal family name. This single word name also is an issue in cultures where names may be more than 1 word, translated into English like my own in Chinese. 垚琰 is my given name and when translated using pingyin, it becomes two words with a space “Yao Yan”. In anglo naming convention, the second name with a space would typically be a middle name.

Several databases also employ a gender binary, which does not include space for third gender, two-spirit, or other gender identities. In some cases, having an other option to write in gender may be more inclusive than limiting defined options even if that makes the data messier.

Healthcare is another field where databases are critical. Joseph A. Goguen in his article “The Ethics of Databases” writes that “there is a tendency to regard only what is explicitly represented in a database as "real," with the rest being implicit, or in the terminology of Bowker and Star, relegated to an invisible infrastructure. Even if some hospital administrators believe that something not represented in a database is important, they will have great difficulty in justifying this belief unless relevant figures can be included in the reports that are read by administrators higher up the "chain of translations.” Depending on what information and categories are included in such healthcare databases, it says a lot about what information we consider relevant to a person’s health outcomes and the way health is siloed rather than treated holistically.  

Common search engines like Google also process queries in a way that highlights values in society. For example, pushing up results that are paid, as well as their new AI summary tool at the front. Tracking queries from the population becomes its own dataset, providing information about what people are thinking about, interested in, and current trends in society. This data can be used to refine search results but can also be used by other third parties for analytics like advertisers. While I do not know if SQL is used in Google, this example highlights that queries are also not free from values systems of those who create databases, maintain it, and use it. 
```
